...
export default function App() {
  ...
  const getapi = async () => {
    try {
      const res = await fetch('http://10.24.46.120:3000/product')
  ...
  const render = ({ item }) => {
    return (
      <View style={{borderWidth:1,width:'90%',alignSelf:'center',borderRadius:10,marginTop:10}}>
        ...
        <Text style={{marginRight:'10%',color:'#FF0000'}} onPress={()=>openupdate(item)}>Update</Text>
          </View>
        <View>
          <Button title='Delete'  color={'orange'}onPress={()=>xoa(item)}/>
        </View>
      </View>
    )
  }
  ...
  const themorsua=()=>{
    ...
    let method='POST';
    let url='http://10.24.46.120:3000/product'
    if(update==true){
      method='PUT';
      url=`http://10.24.46.120:3000/product/${select.id}`
    }
    fetch(url, {
      method: method,
      headers: {
    ...
    Alert.alert(`xóa công việc`,`Bạn có muốn xóa công việc ${item.title}`,[
      {
        text: 'ok',
      onPress:()=>{
        fetch(`http://10.24.46.120:3000/product/${item.id}`, {
          method: 'DELETE',
        }).then((res)=>{
          if(res.status==200){
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={{ flex: 1 }}>
        <Text style={st.tx}>Danh Sách Sản Phẩm</Text>
        <FlatList
          renderItem={render}
          data={data} />
      </View>
      ...
      <View >
      <Text>Số công việc đã hoàn thành: {statusCounts.status1}</Text>
      <Text>Số công việc chưa hoàn thành: {statusCounts.status0}</Text>
      ...
      <Modal visible={add == true}>
        ...
        <View style={{ width: '75%', alignSelf: 'center', marginTop: 10 }}>
          <Button title='Add' color={'blue'} onPress={themorsua}/>
        </View>
        <View style={{ width: '75%', alignSelf: 'center', padding: 0, marginTop: 10 }}>
          <Button title='Cancel' color={'gray'} onPress={()=>setadd(false)}/>
        </View>
      </Modal>

      <Modal visible={update == true}>
        <Text style={st.tx1}>Update công việc</Text>
        <Text>Title:</Text>
        <TextInput style={st.tip} value={titlee} onChangeText={(text)=>settitle(text)}/>
        <Text>Content:</Text>
        <TextInput style={st.tip} value={contentt} onChangeText={(text)=>setcontent(text)}/>
        ...
        <TextInput style={st.tip} value={statuss.toString()} onChangeText={(text)=>setstatus(parseFloat(text))}/>
        <View style={{ width: '75%', alignSelf: 'center', marginTop: 10 }}>
          <Button title='Update'  color={'blue'} onPress={themorsua}/>
        </View>
        <View style={{ width: '75%', alignSelf: 'center', padding: 0, marginTop: 10 }}>
          <Button title='Cancel' color={'gray'} onPress={()=>setupdate(false)}/>
        </View>
      </Modal>
...
const st = StyleSheet.create(
  {
    tx: {
      ...
      fontSize: 25, color: '#FF0000', textAlign: 'center', fontStyle: 'normal',fontWeight:'bold', 
